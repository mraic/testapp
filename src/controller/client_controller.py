...
@marshal_with(response_one_user_schema, 200, apply=False)
...
@allow_access
@marshal_with(response_many_accounts_schema, 200, apply=True)
@marshal_with(message_response_schema, 400, apply=True)
def user_transaction_paginate():
    token = request.environ.get('HTTP_AUTHORIZATION', None)

    payload = jwt.decode(
        token, current_app.config.get('JWT_SECRET_KEY'), algorithms=["HS256"])

    items = AccountService.get_all_accounts_email_sender(user_id=payload['id'])
    return dict(data=items,
                message=Status.successfully_processed().message)


@doc(description='Paginate user orders route', params=security_params,
     tags=['User'])
@bpp.get('/user/account-transaction/paginate')
@allow_access
